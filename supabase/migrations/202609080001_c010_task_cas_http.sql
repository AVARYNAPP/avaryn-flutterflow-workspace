...
-- A stale client row version is an application conflict, not a PostgreSQL
-- serialization failure. 40001 causes the REST transaction layer to retry the
-- same unchanged stale input; PT409 returns an explicit HTTP 409 immediately.
-- Preserve the established authorization-before-receipt ordering and every
-- task mutation/event/idempotency rule from 202609070001 unchanged.
