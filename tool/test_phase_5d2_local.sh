...
if [ "${1:-}" != '--confirm-local-reset' ]; then
  echo 'Refusing Phase 5D.2 reset. Pass --confirm-local-reset explicitly.' >&2
  exit 2
fi
...
reset_log=$(mktemp "${TMPDIR:-/tmp}/avaryn-phase5d2-reset.XXXXXX")
  ...
  reset_state=$(docker exec "$db_container" \
    ...
    "select (select count(*) from auth.users)::text || ':' ||
      (select count(*) from supabase_migrations.schema_migrations
       where version = '202607280004')::text;")
...
echo 'PASS: Phase 5D.2 local feeding gate is green.'
