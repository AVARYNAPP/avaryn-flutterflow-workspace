...
Future<void> _retryOperationalSecurityCleanup() async {
  ...
  try {
    for (final entry in Map<RealtimeChannel, SupabaseClient>.from(
...
List<Map<String, dynamic>> _operationalRows(dynamic value) => value is List
    ? value
        .whereType<Map>()
...
DateTime? _operationalDate(dynamic value) => value is DateTime
    ? value
    : value is String
        ? DateTime.tryParse(value)
        : null;
...
class _AvarynOperationalRuntimeState extends State<AvarynOperationalRuntime> {
  ...
  String get _storagePrefix {
    final userId = _boundUserId.isNotEmpty
        ? _boundUserId
  ...
  Future<void> _load({bool quiet = false}) async {
    ...
    try {
      ...
      if (_requiresSecurityReset(error)) {
        await _purgeOperationalState(clearSelectedStable: true);
        _permissionDenied = true;
        _error = 'Je toegang is gewijzigd. Lokale werkdata is direct gewist; '
  ...
  Future<void> _refreshAuthorityAndTopics() async {
    final cachedAuthority = int.tryParse(
  ...
  Future<void> _fetchOperationalData() async {
    ...
    final results = await Future.wait<dynamic>([
      ...
      _client.rpc(
        ...
        params: {
          'p_stable_id': _stableId,
          'p_from_local_date': widget.mode == 'planning'
              ? _operationalDateKey(range.$1)
              : selectedDate,
          'p_through_local_date': widget.mode == 'planning'
  ...
  Future<void> _fetchFeedingManagementData() async {
    ...
    final versionIds = _feedingPlanVersions
        ...
        .where((id) => id.isNotEmpty)
        .toList(growable: false);
    _feedingPlanItems = versionIds.isEmpty
        ? const []
        : _operationalRows(
            await _client
    ...
    if (!selectedVersions.any(
      ...
      _selectedFeedingVersionId = selectedVersions.isEmpty
          ? ''
  ...
  Future<void> _ensureStableRoster() async {
    ...
    _stableRoster = rosterRows.map((member) {
  ...
  Future<void> _fetchHorseManagementData() async {
    ...
    _horseMedia = _canViewSelectedHorseMedia
  ...
  Future<List<Map<String, dynamic>>> _fetchLinkedMedia({
    ...
    query = scheduleExecutionId.isNotEmpty
        ? query.eq('schedule_execution_id', scheduleExecutionId)
        : query.eq('horse_id', horseId).isFilter('schedule_execution_id', null);
  ...
  Future<void> _prepareOfflineDayset() async {
    if (kIsWeb) {
      setState(() {
        _error = 'Veilige offline dagsets vereisen OS-backed sleutelopslag en '
    ...
    if (!_selectedScheduleDateIsToday) {
      setState(() {
        _error = 'Ga terug naar Vandaag voordat je de versleutelde offline '
            'dagset voorbereidt.';
      });
      return;
    }
  ...
  Future<void> _queueOfflineCompletion(
    ...
    if (kIsWeb) {
      setState(() {
        _error = 'Offline uitvoeringen vereisen de mobiele of desktop-app met '
  ...
  Future<void> _flushOfflineMutations() async {
    ...
    for (final mutation in List<Map<String, dynamic>>.from(queued)) {
      ...
      final params = {
        ...
        'p_note': _operationalString(mutation['comments']).isEmpty
            ? null
            : mutation['comments'],
      };
      try {
  ...
  Uint8List _buildMediaThumbnail(Uint8List bytes, String mimeType) {
    final image.Decoder decoder = switch (mimeType) {
      ...
      _ => throw const _OperationalMediaException(
          400,
  ...
  Future<void> _pickAndUploadMedia({
    ...
    if (file.size <= 0 || file.size > maxBytes) {
      setState(
        () => _error = mimeType == 'application/pdf'
    ...
    await _guarded(() async {
      final thumbnail = mimeType == 'application/pdf'
          ? null
      ...
      _notice = scheduleExecutionId.isEmpty
  ...
  Future<void> _uploadMediaBytes({
    ...
    try {
      ...
      for (final upload in uploads) {
        ...
        final variantBytes = variant == 'original'
            ? original
            : variant == 'thumbnail'
                ? thumbnail
                : null;
        ...
        try {
          await _client.storage.from('horse-media').uploadBinaryToSignedUrl(
                objectPath,
                uploadToken,
                variantBytes,
  ...
  Future<void> _archiveMediaAsset(Map<String, dynamic> asset) async {
    ...
    final confirmed = await _confirmDialog(
      ...
      body: 'Archiveer ${_operationalString(asset['original_filename'])}? '
  ...
  Future<void> _removeSelectedHorseProfilePhoto() async {
    ...
    final confirmed = await _confirmDialog(
      ...
      body: 'Verwijder de foto uit het paardprofiel? De beveiligde '
                      ...
                      trailing: Row(
                        ...
                        children: [
                          IconButton(
                            ...
                            onPressed: () => unawaited(_openMediaAsset(asset)),
  ...
  Future<Map<String, dynamic>?> _showFeedingExecutionDialog({
    ...
    var unitCode = _operationalString(item?['planned_unit_code']).isEmpty
        ? 'portion'
            ...
            actions: [
              ...
              FilledButton(
                onPressed: () {
                  ...
                  final remainingValue = remaining.text.trim().isEmpty
                      ? null
                      : double.tryParse(
    ...
    await _guarded(() async {
      ...
      if (isFeeding) {
        ...
        await _runDurableIdempotentRpc(
          ...
          intentKey: '$scheduleItemId:${feedingDetails!['actual_quantity']}:'
          ...
          buildParams: (requestId, replayValues) => {
            ...
            'p_note': replayValues['comments']!.isEmpty
                ? null
            ...
            'p_remaining_quantity': replayValues['remaining_quantity']!.isEmpty
                ? null
                : double.parse(replayValues['remaining_quantity']!),
            'p_deviation_code': replayValues['deviation_code'],
            'p_observation': replayValues['observation']!.isEmpty
                ? null
  ...
  Future<void> _correctFeedingExecution(Map<String, dynamic> item) async {
    ...
    if (originalExecutionId.isEmpty || scheduleItemId.isEmpty) {
      setState(() {
        _error = 'De oorspronkelijke uitvoering is niet beschikbaar. Vernieuw '
    ...
    await _guarded(() async {
      ...
      await _runDurableIdempotentRpc(
        operation: 'record_feeding_execution',
        intentKey: 'correction:$originalExecutionId:'
  ...
  Future<Map<String, dynamic>?> _showHorseProfilePage({
    ...
    var sex = const {'mare', 'gelding', 'stallion'}.contains(horse?['sex'])
    ...
    final canChooseRelationships = horse == null
      ...
      MaterialPageRoute(
        ...
        builder: (routeContext) => StatefulBuilder(
          builder: (context, setPageState) {
            final liveHorse = horse == null ? null : (_selectedHorse ?? horse);
            Map<String, dynamic>? normalizedProfile() {
              ...
              final parsedBirth = normalizedBirth.isEmpty
                  ? null
                  : _operationalParseDisplayDate(normalizedBirth);
              final parsedExpiry = normalizedExpiry.isEmpty
                  ? null
              ...
              if (normalizedBirth.isNotEmpty &&
                  (parsedBirth == null || parsedBirth.isAfter(currentDay))) {
                validationError = 'Gebruik voor de geboortedatum DD-MM-JJJJ.';
                return null;
              }
              if (normalizedExpiry.isNotEmpty && parsedExpiry == null) {
                validationError = 'Gebruik voor de paspoortdatum DD-MM-JJJJ.';
                return null;
              }
              validationError = null;
              return {
                'display_name': normalizedName,
                'official_name': officialName.text.trim().isEmpty
                    ? null
                    : officialName.text.trim(),
                'birth_date': parsedBirth == null
                    ? null
                    : _operationalDateKey(parsedBirth),
                'sex': sex,
                'breed': breed.text.trim().isEmpty ? null : breed.text.trim(),
                'discipline': selectedDiscipline,
                'level': _operationalString(horse?['level']).isEmpty
                    ? null
                    : _operationalString(horse?['level']),
                'passport_number': passportNumber.text.trim().isEmpty
                    ? null
                    : passportNumber.text.trim(),
                'chip_number': chipNumber.text.trim().isEmpty
                    ? null
                    : chipNumber.text.trim(),
                'passport_expires_on': parsedExpiry == null
                    ? null
                    : _operationalDateKey(parsedExpiry),
                'owner_member_id': ownerMemberId.isEmpty ? null : ownerMemberId,
                'trainer_member_id':
                    trainerMemberId.isEmpty ? null : trainerMemberId,
                'rider_member_id': riderMemberId.isEmpty ? null : riderMemberId,
              };
            }

            Widget fieldLabel(String label, {bool required = false}) => Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              if (step == 0) ...[
                                ...
                                DropdownButtonFormField<String>(
                                  value: selectedDiscipline.isEmpty
                                      ? null
                                      : selectedDiscipline,
                                  dropdownColor: panel,
                                  decoration: const InputDecoration(
                      ...
                      Container(
                        ...
                        child: SizedBox(
                          ...
                          child: FilledButton.icon(
                            onPressed: () {
                              if (step < 2) {
                                if (step == 0 && normalizedProfile() == null) {
                                  setPageState(() {});
                                  return;
                                }
                                setPageState(() {
    ...
    await _guarded(() async {
      await _runDurableIdempotentRpc(
        ...
        buildParams: (requestId, _) => {
          ...
          'p_can_view': true,
          'p_can_execute': grant['access_level'] == 'work' &&
              grant['category'] == 'horse.schedule',
          'p_can_edit': grant['access_level'] == 'work' &&
              const {
                'horse.basic',
                'horse.media',
              ...
              child: Column(
                ...
                children: [
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      labelText: category == 'horse.media'
  ...
  Future<void> _createScheduleItem() async {
    ...
    final localTime = '${start.hour.toString().padLeft(2, '0')}:'
      ...
      final created = _operationalMap(
        await _runDurableIdempotentRpc(
          ...
          buildParams: (requestId, replayValues) => {
            ...
            'p_assignment_request_id': replayValues['assignment_request_id'],
          },
        ),
      );
  ...
  Future<Map<String, String>?> _showScheduleTaskDialog() async {
    ...
    var horseId = _selectedHorseId.isNotEmpty
        ? _selectedHorseId
                        ...
                        Expanded(
                          child: DropdownButtonFormField<int>(
                            ...
                            items: const [0, 15, 30, 45]
                                ...
                                .toList(growable: false),
                            onChanged: (value) => setDialogState(
                              () => selectedMinute = value ?? selectedMinute,
                            ),
                          ),
                        ),
                      ],
              ...
              FilledButton(
                onPressed: () {
                  ...
                  Navigator.pop(dialogContext, {
                    ...
                    'item_kind': activityType == 'Training'
                        ? 'training'
                    ...
                    'instruction': instruction.text.trim().isEmpty
  ...
  Future<void> _createScheduleSeries() async {
    ...
    final localTime = '${hour.toString().padLeft(2, '0')}:'
      ...
      final created = _operationalMap(
        await _runDurableIdempotentRpc(
          ...
          buildParams: (requestId, replayValues) => {
            ...
            'p_interval_value': 1,
            'p_weekdays': replayValues['frequency'] == 'weekly'
            ...
            'p_stable_member_id': replayValues['stable_member_id']!.isEmpty
                ? null
                : replayValues['stable_member_id'],
            'p_create_request_id': requestId,
            'p_materialize_request_id': replayValues['materialize_request_id'],
          },
        ),
      );
  ...
  Widget _scheduleDateControls(FlutterFlowTheme theme) {
    ...
    final step = _calendarView == 'month'
      ...
      children: [
        ...
        Row(
          children: [
            ...
            Expanded(
              child: TextButton(
                onPressed: _offline || _busy || isToday
                    ? null
  ...
  Widget _calendarControls(FlutterFlowTheme theme) => SegmentedButton<String>(
        ...
        onSelectionChanged: _offline || _busy
            ? null
    ...
    await _guarded(() async {
      final created = _operationalMap(
        await _runDurableIdempotentRpc(
          ...
          intentKey: '${input['horse_id']}:${input['plan_type']}:'
          ...
          buildParams: (requestId, replayValues) => {
            ...
            'p_effective_until': replayValues['effective_until']!.isEmpty
                ? null
                ...
                children: [
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      errorText:
                          showNameError ? 'Vul een duidelijke naam in.' : null,
                    ),
                  ),
              ...
              FilledButton(
                onPressed: () {
                  ...
                  Navigator.pop(dialogContext, {
                    ...
                    'effective_until':
                        planType == 'temporary' ? _operationalDateKey(end) : '',
                    'change_reason': planType == 'temporary'
  ...
  List<String> get _activeStandardOverrideKeys {
    final activeVersionIds = _feedingPlans
        .where(
          (plan) =>
  ...
  Future<void> _upsertFeedingItem([Map<String, dynamic>? existing]) async {
    ...
    if (temporary && overrideKeys.isEmpty) {
      setState(() {
        _error = 'Activeer eerst een standaardplan met een voerslot voordat je '
    ...
    await _guarded(() async {
      await _runDurableIdempotentRpc(
        ...
        intentKey: '$versionId:${itemId.isEmpty ? 'new' : itemId}:'
        ...
        buildParams: (requestId, replayValues) => {
          ...
          'p_feeding_plan_item_id':
              replayValues['item_id']!.isEmpty ? null : replayValues['item_id'],
          'p_expected_row_version': replayValues['row_version']!.isEmpty
              ? null
              : int.parse(replayValues['row_version']!),
          'p_item_category': replayValues['item_category'],
          'p_product_brand': replayValues['product_brand']!.isEmpty
              ? null
          ...
          'p_batch_lot': null,
          'p_expires_on': null,
          'p_instruction': replayValues['instruction']!.isEmpty
              ? null
  ...
  Future<Map<String, String>?> _showFeedingItemDialog({
    ...
    var unitCode = _operationalString(existing?['unit_code']).isEmpty
        ? 'kg'
        : _operationalString(existing?['unit_code']);
    var itemCategory = _operationalString(existing?['item_category']).isEmpty
        ? 'feed'
    ...
    var roundCode = _operationalString(existing?['round_code']).isEmpty
        ? 'ochtend'
              ...
              FilledButton(
                onPressed: () {
                  ...
                  final normalizedOverride =
                      temporary ? overrideKey : overrideController.text.trim();
                  if (normalizedName.isEmpty ||
                      parsedQuantity == null ||
                      parsedQuantity <= 0 ||
                  ...
                  Navigator.pop(dialogContext, {
                    ...
                    'local_time': '${selectedHour.toString().padLeft(2, '0')}:'
  ...
  Future<void> _approveSelectedFeedingVersion() async {
    ...
    final confirmed = await _confirmSensitiveFeedingAction(
      ...
      body: 'Na goedkeuring zijn de voeritems immutable. Maak voor latere '
  ...
  Future<void> _activateSelectedFeedingVersion() async {
    ...
    final confirmed = await _confirmSensitiveFeedingAction(
      ...
      body: 'Dit materialiseert de komende 30 lokale dagen en vervangt alleen '
  ...
  Future<bool> _purgeOperationalState({
    ...
    final purgeAccountId = _boundUserId.isNotEmpty
        ? _boundUserId
    ...
    final horseId = await showDialog<String>(
      ...
      builder: (dialogContext) => StatefulBuilder(
        builder: (context, setDialogState) {
          ...
          final matches = _horses.where((horse) {
                        ...
                        : ListView.separated(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return ListTile(
                                ...
                                subtitle: officialName.isEmpty
                                    ? null
                                    : Text(officialName),
                                trailing: _selectedHorseId ==
                                        _operationalString(
                                          horse['id'],
                                        )
    ...
    return Column(
      ...
      children: [
        ...
        SegmentedButton<String>(
          ...
          onSelectionChanged: (values) =>
              setState(() => _horseProfileTab = values.single),
        ),
        const SizedBox(height: 16),
  ...
  Future<Map<String, String>?> _showScheduleEditDialog(
    ...
    var priority = _operationalString(item['priority']).isEmpty
        ? 'normal'
    ...
    await _guarded(() async {
      if (seriesId.isEmpty) {
        await _runDurableIdempotentRpc(
          ...
          intentKey: '${item['schedule_item_id']}:${item['row_version']}:'
          ...
          initialReplayValues: {
            ...
            'scheduled_end_at': localStart
                .add(const Duration(minutes: 30))
                .toUtc()
                .toIso8601String(),
            'source_local_date': _operationalDateKey(localStart),
            'source_local_time': '${hour.toString().padLeft(2, '0')}:'
                '${minute.toString().padLeft(2, '0')}:00',
          },
          scopePreflight: scopePreflight,
          buildParams: (requestId, replayValues) => {
        ...
        await _runDurableIdempotentRpc(
          ...
          initialReplayValues: {
            ...
            'title': input['title']!,
            'instruction': instruction,
            'local_start_time': '${hour.toString().padLeft(2, '0')}:'
                '${minute.toString().padLeft(2, '0')}:00',
            'materialize_request_id': _uuid.v4(),
          ...
          buildParams: (requestId, replayValues) => {
            ...
            'p_occurrence_item_id': replayValues['schedule_item_id']!.isEmpty
                ? null
            ...
            'p_instruction': replayValues['instruction'],
            'p_timezone': _stableTimezone,
            'p_frequency': _operationalString(item['series_frequency']).isEmpty
                ? 'daily'
                : item['series_frequency'],
            'p_interval_value': int.tryParse(
                  _operationalString(item['series_interval_value']),
                ) ??
                1,
            'p_weekdays': item['series_weekdays'],
            'p_local_start_time': replayValues['local_start_time'],
            'p_duration_minutes': int.tryParse(
                  _operationalString(item['series_duration_minutes']),
                ) ??
                30,
            'p_ends_on': _operationalString(item['series_ends_on']).isEmpty
                ? null
                : item['series_ends_on'],
            'p_generation_horizon_days': 45,
            'p_status': _operationalString(item['series_status']).isEmpty
                ? 'active'
                : item['series_status'],
  ...
  Future<void> _recordScheduleProgress(
    ...
    await _guarded(() async {
      ...
      await _runIdempotentRpc(
        operation: 'record_schedule_execution',
        intentKey: '${item['schedule_item_id']}:$status:'
  ...
  List<Map<String, dynamic>> _filteredScheduleRows({bool feedingOnly = false}) {
    ...
    final rows = _schedule.where((item) {
      ...
      if (_calendarCategoryFilter.isNotEmpty &&
          _operationalString(item['item_kind']) != _calendarCategoryFilter) {
        return false;
      }
    ...
    rows.sort((left, right) {
      final leftTime = _operationalDate(left['scheduled_start_at']) ??
          DateTime.fromMillisecondsSinceEpoch(0);
      final rightTime = _operationalDate(right['scheduled_start_at']) ??
          DateTime.fromMillisecondsSinceEpoch(0);
  ...
  Widget _scheduleVisualCard(
    ...
    final responsible = _operationalString(item['responsible_name']).isEmpty
        ? 'Niet toegewezen'
        : _operationalString(item['responsible_name']);
  ...
  Widget _scheduleContent(
    ...
    return Column(
      ...
      children: [
        ...
        if (!kIsWeb)
          OutlinedButton.icon(
            onPressed: _offline || _busy || !_selectedScheduleDateIsToday
                ? null
  ...
  Widget _feedingContent(FlutterFlowTheme theme) {
    if (_feedingPlans.isEmpty) {
      return _emptyCard(
        ...
        body: 'Maak een conceptplan aan. Goedkeuring en activatie blijven '
      ...
      children: [
          ...
          (plan) => _dataCard(
            ...
            onTap: _busy
                ...
                : () {
                    ...
                    setState(() {
                      _selectedFeedingPlanId = planId;
                      _selectedFeedingVersionId = versions.isEmpty
                          ? ''
            ...
            child: Column(
              ...
              children: [
                ...
                if (selectedVersion == null)
                  ...
                  for (final roundCode in selectedItems
                    ...
                    ...selectedItems
                        ...
                        .map(
                          (item) => _dataCard(
                            ...
                            trailing: versionStatus == 'draft'
                                ? 'Wijzigen'
                                : 'In schema',
                            onTap: _offline || _busy || versionStatus != 'draft'
                                ? null
        ...
        if (selectedPlan != null) ...[
          ...
          if (historyRows.isEmpty)
            ...
            ...historyRows.take(20).map(
                  (item) => _dataCard(
                    theme,
                    leading: _operationalString(item['state']) == 'skipped'
                        ? Icons.block_outlined
                        : Icons.history_outlined,
                    title: _operationalString(item['product_name']).isEmpty
                        ? _operationalString(item['title'])
                        : _operationalString(item['product_name']),
                    subtitle:
                        '${_operationalDisplayDate(item['source_local_date'])} · '
                    ...
                    trailing: _operationalString(item['state']) == 'skipped'
                        ? 'Gemist'
                        : 'Afgetekend',
            ...
            child: Row(
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    color:
                        (accentColor ?? theme.primary).withValues(alpha: 0.10),
                    borderRadius: BorderRadius.circular(12),
                  ),
                  child: Icon(
                    leading,
      ...
      Container(
        ...
        child: Row(
          children: [
            ...
            if (error)
              TextButton(onPressed: _load, child: const Text('Opnieuw')),
          ],
        ),
      );
